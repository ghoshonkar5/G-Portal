...
interface ScopusPublication {
  ...
  doi: string; link: string; docType: string; source: string; selected?: boolean;
}

interface AuthorProfile { hIndex: number; citationCount: number; documentCount: number; }
interface ScopusSyncModalProps { isOpen: boolean; onClose: () => void; onImportComplete: () => void; }
...
const V1Styles = () => (
  <style dangerouslySetInnerHTML={{__html: `
    ...
    .scopus-v1 {
      --teal: #0F5C58; --brass: #B08A3E; --brass-soft: #F2EADA; --paper: #FBF8F1;
    ...
    .dark .scopus-v1 {
      --teal: #4DB8B0; --brass: #D4A855; --brass-soft: #2A2116; --paper: #1A1916;
      --ink: #F0EDE6; --ink-2: #C8C3BB; --ink-3: #8A857E; --ink-4: #504C48;
      --rule: #2E2C28; --rule-2: #3A3630; --rose: #E07070; --rose-soft: #2A1515;
    }
    
    .scopus-v1 .serif { font-family: 'Source Serif 4', Georgia, serif; }
    .scopus-v1 .mono { font-family: 'JetBrains Mono', monospace; }
    
    .scopus-v1 .v-rule { width: 1px; background: var(--rule); align-self: stretch; }
    .scopus-v1 .dot { width: 6px; height: 6px; border-radius: 50%; display: inline-block; }
    
    .scopus-v1 .btn { transition: all 0.15s ease; cursor: pointer; }
    .scopus-v1 .btn:hover:not(:disabled) { transform: translateY(-1px); }
    .scopus-v1 .btn:active:not(:disabled) { transform: translateY(0); }
    .scopus-v1 .btn:disabled { opacity: 0.5; cursor: not-allowed; transform: none; }
    
    .scopus-v1 .scroll::-webkit-scrollbar { width: 6px; }
    .scopus-v1 .scroll::-webkit-scrollbar-track { background: transparent; }
    .scopus-v1 .scroll::-webkit-scrollbar-thumb { background: var(--rule); border-radius: 4px; }
    .scopus-v1 .scroll::-webkit-scrollbar-thumb:hover { background: var(--ink-4); }

    .scopus-v1 input[type="checkbox"].sq {
      appearance: none; -webkit-appearance: none;
      width: 14px; height: 14px; border: 1px solid var(--ink-4);
      border-radius: 2px; background: var(--paper); cursor: pointer;
      position: relative; flex-shrink: 0; margin: 0;
    }
    .scopus-v1 input[type="checkbox"].sq:checked { background: var(--teal); border-color: var(--teal); }
    .scopus-v1 input[type="checkbox"].sq:checked::after {
    ...
    .scopus-v1 .shimmer-bar { height: 4px; border-radius: 0; overflow: hidden; background: var(--rule-2); position: relative; }
    .scopus-v1 .shimmer-bar > i {
...
export function ScopusSyncModal({ isOpen, onClose, onImportComplete }: ScopusSyncModalProps) {
  ...
  const importStartTime = useMemo(() => Date.now(), [step]);

  const allScopusUrls = useMemo(() => [user?.scopusUrl, (user as any)?.scopusUrl2, (user as any)?.scopusUrl3].filter(Boolean) as string[], [user]);

  useEffect(() => { if (isOpen && facultyId) fetchLastSynced(); }, [isOpen, facultyId]);

  const fetchLastSynced = async () => {
    try {
      const res = await fetch(`${API_BASE_URL}/scopus/last-synced/${facultyId}`, { headers: { Authorization: `Bearer ${getToken()}` } });
  ...
  const handleManualSync = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setErrorMsg(data.message || `Sync failed (HTTP ${res.status})`);
        setStep('error');
      }
  ...
  const fetchPreview = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/scopus/preview/${facultyId}`, { headers: { Authorization: `Bearer ${getToken()}` } });
      const data = await res.json();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-[880px] p-0 border-0 bg-transparent shadow-none overflow-hidden [&>button]:hidden">
        ...
        <div className="scopus-v1 flex flex-col rounded shadow-2xl border border-[var(--rule)] relative">
            ...
            <div style={{ padding: '22px 32px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 24 }}>
              <div>
                ...
                <h1 className="serif" style={{ fontSize: 32, fontWeight: 500, lineHeight: 1.1, margin: 0, color: 'var(--ink)', letterSpacing: '-0.01em' }}>
                  Sync from <em style={{ fontStyle: 'italic', color: 'var(--teal)' }}>Scopus</em>
                </h1>
              </div>
          ...
          <div style={{ padding: '28px 32px 32px' }}>
            ...
            {step === 'idle' && (
              <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1px 1fr', gap: 32 }}>
                <div>
                  <MonoLabel>Connected Scopus profiles · {allScopusUrls.length}</MonoLabel>
                  <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {allScopusUrls.map((url, i) => (
                      <a key={i} href={url} target="_blank" rel="noreferrer"
                        ...
                        <span className="mono" style={{ fontSize: 12, color: 'var(--ink-2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>{url}</span>
                        <ArrowUpRight size={14} style={{ color: 'var(--ink-4)' }} />
                      </a>
                    ))}
                    {allScopusUrls.length === 0 && (
                  ...
                  {allScopusUrls.length > 1 && (
                    <div style={{ marginTop: 12, display: 'flex', alignItems: 'center', gap: 8 }}>
                      <GitMerge size={12} style={{ color: 'var(--teal)' }} />
                      <span className="mono" style={{ fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--teal)' }}>Merged · Deduplicated on DOI</span>
                    </div>
                  )}
                  <div style={{ marginTop: 22, padding: '14px 16px', borderLeft: '2px solid var(--brass)', background: 'var(--brass-soft)' }}>
                    ...
                    <p style={{ margin: 0, fontSize: 13, color: 'var(--ink-2)', lineHeight: 1.5 }}>
                      You must be connected to the <strong>college network or VPN</strong>. The Scopus API requires institutional access.
                    </p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div style={{ marginTop: 14, padding: '18px 18px', border: '1px solid var(--rule)', borderRadius: 2, background: 'var(--paper)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <span className="dot" style={{ background: 'var(--teal)', boxShadow: '0 0 0 4px rgba(15,92,88,0.12)' }} />
                    ...
                    <div style={{ marginTop: 14, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                      ...
                      <div><MonoLabel>Window</MonoLabel><div className="serif" style={{ fontSize: 15, marginTop: 4, color: 'var(--ink)' }}>02:00 IST</div></div>
                      ...
                      <div><MonoLabel>Next run</MonoLabel><div className="mono" style={{ fontSize: 12, marginTop: 4, color: 'var(--ink-2)' }}>Tonight · 02:00</div></div>
                    </div>
                  </div>

                  <MonoLabel style={{ marginTop: 22 }}>Manual actions</MonoLabel>
                  <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <button className="btn" onClick={handleManualSync} disabled={allScopusUrls.length === 0}
                    ...
                    <button className="btn" onClick={fetchPreview} disabled={loadingPreview || allScopusUrls.length === 0}
                      style={{ padding: '14px 18px', background: 'transparent', color: 'var(--teal)', border: '1px solid var(--teal)', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: 14, fontWeight: 500 }}>
                      <span style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 0' }}>
                  ...
                  <div style={{ display: 'flex', gap: 18 }}>
                    <button onClick={() => toggleAll(true)} className="mono" style={{ fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', background: 'transparent', border: 0, color: 'var(--teal)', cursor: 'pointer' }}>Select all</button>
                ...
                <div className="scroll" style={{ maxHeight: 380, overflowY: 'auto', border: '1px solid var(--rule)' }}>
                  {publications.map((p, i) => (
                    <div key={i} onClick={() => togglePub(i)}
                      style={{ display: 'grid', gridTemplateColumns: '24px 60px 1fr auto', gap: 16, padding: '16px 20px', cursor: 'pointer', borderBottom: i < publications.length - 1 ? '1px solid var(--rule-2)' : 'none', background: p.selected ? 'rgba(15,92,88,0.04)' : 'transparent' }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        ...
                        <div style={{ marginTop: 6, display: 'flex', flexWrap: 'wrap', gap: 14, alignItems: 'center' }}>
                          ...
                          {p.citations > 0 && <span className="mono" style={{ fontSize: 10, letterSpacing: '0.08em', color: 'var(--teal)' }}>{p.citations} citations</span>}
                        </div>
                        <div className="mono" style={{ fontSize: 11, color: 'var(--ink-4)', marginTop: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.authors}</div>
                      </div>
                      {p.link && (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 22 }}>
                  ...
                  <button onClick={handleImport} disabled={selectedCount === 0} className="btn" style={{ background: 'var(--teal)', color: 'var(--paper)', border: '1px solid var(--teal)', padding: '10px 22px', borderRadius: 2, fontSize: 13, fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ padding: '32px 0 18px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 26 }}>
                  ...
                  <div>
                    ...
                    <h2 className="serif" style={{ fontSize: 22, fontWeight: 500, margin: '4px 0 0', color: 'var(--ink)' }}>Importing publications from Scopus…</h2>
                  </div>
                </div>
            ...
            {step === 'done' && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1px 1fr', gap: 36, alignItems: 'center', padding: '24px 0' }}>
                <div>
                  <MonoLabel style={{ color: 'var(--teal)' }}>Sync complete</MonoLabel>
                  <h2 className="serif" style={{ fontSize: 36, fontWeight: 500, margin: '12px 0 12px', color: 'var(--ink)', lineHeight: 1.05, letterSpacing: '-0.01em' }}>
                    ...
                    {importStats.new > 0 && <em style={{ color: 'var(--teal)', fontStyle: 'italic' }}>added to your catalog.</em>}
                  </h2>
                  ...
                  <button onClick={handleClose} className="btn" style={{ marginTop: 24, background: 'var(--teal)', color: 'var(--paper)', border: '1px solid var(--teal)', padding: '11px 24px', borderRadius: 2, fontSize: 13, fontWeight: 500 }}>
                    Done
                  </button>
                </div>
                ...
                <div>
                  ...
                  <div style={{ marginTop: 14, display: 'grid', gap: 14 }}>
                    {[
                      ...
                      ['Next auto-sync', 'Tonight · 02:00 IST'],
                    ].map(([k, v]) => (
            ...
            {step === 'error' && (
              <div>
                <div style={{ borderLeft: '3px solid var(--rose)', padding: '20px 24px', background: 'var(--rose-soft)' }}>
                  ...
                  <h2 className="serif" style={{ fontSize: 24, fontWeight: 500, margin: '6px 0 10px', color: 'var(--ink)' }}>Could not complete Scopus sync.</h2>
                ...
                <div style={{ marginTop: 24 }}>
                  ...
                  <ol style={{ margin: '12px 0 0', padding: 0, listStyle: 'none', display: 'grid', gap: 10 }}>
                    {['Confirm you are on the institutional network or VPN.', 'Open scopus.com in a new tab — it should load without prompting for a login.', 'If you recently changed your Scopus profile URL, save it in Profile Settings.', 'If the problem persists, contact the research office with the timestamp below.'].map((t, i) => (
                  ...
                  <div className="mono" style={{ marginTop: 16, fontSize: 11, color: 'var(--ink-4)' }}>
                    ref: scopus-sync · {new Date().toISOString()} · faculty {facultyId}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginTop: 26 }}>
                  ...
                  <button onClick={handleManualSync} className="btn" style={{ background: 'var(--teal)', color: 'var(--paper)', border: '1px solid var(--teal)', padding: '10px 22px', borderRadius: 2, fontSize: 13, fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: 10 }}>
