...
export default function Navbar({ isAdmin = false }: NavbarProps) {
  ...
  let portalTitle = 'EVENTS PORTAL'
  let portalSubtitle = `${roleLabel} Event Management System`
        ...
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden md:block text-right">
              ...
              <div className="text-xs text-[#BAE6FD] opacity-90 capitalize">
                {isAdmin ? 'System Administrator' : [user?.universityId || user?.faculty_id || user?.UniversityId || roleLabel, user?.department || user?.branch].filter(Boolean).join(' · ')}
              </div>
            </div>
